...
- 重整市场导航、筛选工具栏、插件卡片和管理面板，统一按钮尺寸、焦点反馈与明暗主题；
  窄窗口下操作区自动换行，卸载与常用操作在视觉上区分。
- 增加搜索结果计数和筛选重置，完善加载及空结果反馈；已选插件跨筛选保留，明确提示隐藏的
  已选项，使按钮计数、确认信息和实际批量操作范围一致。
- 显式刷新 Registry 保留最近有效快照与 ETag，网络故障时不再退回更旧数据；并发刷新合并请求。
- 核心 Registry 返回 304 时仍更新分类、热度及查找索引；包内快照不可读时自动尝试远端。
- GitHub 自动选择的 Release tag 或默认分支先解析为精确 commit，再统一读取 manifest、patch 和入口，
  避免跨 commit 混读，并移除手动安装中重复的文件请求。
- 支持通过 `exports` 封装且未公开 `./package.json` 的插件，正确读取版本、Bundle 状态并执行冲突检查。
- Profile 已解除关联后，实体清理失败不再恢复可能已部分删除的插件；日志保留残留路径。
  安装成功后的备份及临时目录清理失败也不再触发回滚。

### 测试

- 增加真实 React 组件与 DSH 基础组件的浏览器回归，使用模拟数据验证响应式布局、主题、筛选、
  选择状态和操作确认，不触碰真实 Profile。
- 补充条件请求、离线恢复、并发刷新、自动 ref 固定、封装 manifest，以及 Windows 文件占用与部分删除回归。
